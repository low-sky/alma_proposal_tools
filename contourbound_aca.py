...
def imglist(datadir='/mnt/work/erosolow/phangs/proposal/for_er/'):
    return(glob.glob(datadir + '*w3_bksub*fits'))
...
def genboxes():
    PixelMask = False
    ll = imglist()
    namelist = np.loadtxt('proposed_aca.txt',dtype='str', delimiter=',')
    if len(namelist.shape) > 1:
        fluxval = np.array(namelist[:, 1], dtype='float')
        namelist = namelist[:, 0]
    t = Table(names=('Galaxy','SB','RA','Dec','Vlsr', 'LongOff',
                     'LatOff','p','q','PA'),
              dtype=('S8','i4','f8','f8','f8','f8',
                     'f8','f8','f8','f8'))
    for flux, name in zip(fluxval, namelist):
        print flux
        l = [l for l in ll if name in l]
        g, box, www = contourbound(l[0], doplot=True,
                                   level=flux * u.MJy/u.sr)
        if box is None:
        ...
        if name == 'PGC50779':
            print "Override ", g.name
            box.position_angle = -60 * np.pi/ 180
